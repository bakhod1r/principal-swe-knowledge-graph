...
## 1. Definition
**Distributed Message Queue Scalability Patterns** defines the horizontal sharding models, multi-tier caching architectures, and throughput scaling strategies for **Distributed Message Queue**.
...
## 3. Usage
```yaml
# Kubernetes & Envoy Horizontal Autoscaling Topology for Distributed Message Queue
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: distributed-message-queue-autoscaler
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
    name: distributed-message-queue-service
  minReplicas: 10
  maxReplicas: 200
  metrics:
  - type: Resource
