...
## 1. Definition
**Google Spanner Scalability Patterns** defines the horizontal sharding models, multi-tier caching architectures, and throughput scaling strategies for **Google Spanner**.
...
## 3. Usage
```yaml
# Kubernetes & Envoy Horizontal Autoscaling Topology for Google Spanner
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: google-spanner-autoscaler
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
    name: google-spanner-service
  minReplicas: 10
  maxReplicas: 200
  metrics:
  - type: Resource
