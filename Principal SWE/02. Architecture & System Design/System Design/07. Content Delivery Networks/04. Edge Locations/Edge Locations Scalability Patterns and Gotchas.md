...
## 1. Definition
**Edge Locations Scalability Patterns** defines the horizontal sharding models, multi-tier caching architectures, and throughput scaling strategies for **Edge Locations**.
...
## 3. Usage
```yaml
# Kubernetes & Envoy Horizontal Autoscaling Topology for Edge Locations
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: edge-locations-autoscaler
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
    name: edge-locations-service
  minReplicas: 10
  maxReplicas: 200
  metrics:
  - type: Resource
