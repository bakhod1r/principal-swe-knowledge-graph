...
## 1. Definition
**Wide Column Scalability Patterns** defines the horizontal sharding models, multi-tier caching architectures, and throughput scaling strategies for **Wide Column**.
...
## 3. Usage
```yaml
# Kubernetes & Envoy Horizontal Autoscaling Topology for Wide Column
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: wide-column-autoscaler
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
    name: wide-column-service
  minReplicas: 10
  maxReplicas: 200
  metrics:
  - type: Resource
