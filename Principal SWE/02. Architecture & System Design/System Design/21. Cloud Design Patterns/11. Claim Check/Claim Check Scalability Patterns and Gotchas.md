...
## 1. Definition
**Claim Check Scalability Patterns** defines the horizontal sharding models, multi-tier caching architectures, and throughput scaling strategies for **Claim Check**.
...
## 3. Usage
```yaml
# Kubernetes & Envoy Horizontal Autoscaling Topology for Claim Check
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: claim-check-autoscaler
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
    name: claim-check-service
  minReplicas: 10
  maxReplicas: 200
  metrics:
  - type: Resource
