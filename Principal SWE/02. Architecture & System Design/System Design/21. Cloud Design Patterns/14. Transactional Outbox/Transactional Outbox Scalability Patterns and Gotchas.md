...
## 1. Definition
**Transactional Outbox Scalability Patterns** defines the horizontal sharding models, multi-tier caching architectures, and throughput scaling strategies for **Transactional Outbox**.
...
## 3. Usage
```yaml
# Kubernetes & Envoy Horizontal Autoscaling Topology for Transactional Outbox
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: transactional-outbox-autoscaler
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
    name: transactional-outbox-service
  minReplicas: 10
  maxReplicas: 200
  metrics:
  - type: Resource
