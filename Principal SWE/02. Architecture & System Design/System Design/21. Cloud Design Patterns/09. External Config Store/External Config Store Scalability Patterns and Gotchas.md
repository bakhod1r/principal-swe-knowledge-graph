...
## 1. Definition
**External Config Store Scalability Patterns** defines the horizontal sharding models, multi-tier caching architectures, and throughput scaling strategies for **External Config Store**.
...
## 3. Usage
```yaml
# Kubernetes & Envoy Horizontal Autoscaling Topology for External Config Store
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: external-config-store-autoscaler
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
    name: external-config-store-service
  minReplicas: 10
  maxReplicas: 200
  metrics:
  - type: Resource
