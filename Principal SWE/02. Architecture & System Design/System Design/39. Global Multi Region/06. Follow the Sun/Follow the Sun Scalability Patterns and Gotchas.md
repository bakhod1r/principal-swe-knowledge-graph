...
## 1. Definition
**Follow the Sun Scalability Patterns** defines the horizontal sharding models, multi-tier caching architectures, and throughput scaling strategies for **Follow the Sun**.
...
## 3. Usage
```yaml
# Kubernetes & Envoy Horizontal Autoscaling Topology for Follow the Sun
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: follow-the-sun-autoscaler
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
    name: follow-the-sun-service
  minReplicas: 10
  maxReplicas: 200
  metrics:
  - type: Resource
