...
## 1. Definition
**Task Queues Scalability Patterns** defines the horizontal sharding models, multi-tier caching architectures, and throughput scaling strategies for **Task Queues**.
...
## 3. Usage
```yaml
# Kubernetes & Envoy Horizontal Autoscaling Topology for Task Queues
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: task-queues-autoscaler
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
    name: task-queues-service
  minReplicas: 10
  maxReplicas: 200
  metrics:
  - type: Resource
