...
## 1. Definition
**Postmortems Scalability Patterns** defines the horizontal sharding models, multi-tier caching architectures, and throughput scaling strategies for **Postmortems**.
...
## 3. Usage
```yaml
# Kubernetes & Envoy Horizontal Autoscaling Topology for Postmortems
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: postmortems-autoscaler
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
    name: postmortems-service
  minReplicas: 10
  maxReplicas: 200
  metrics:
  - type: Resource
