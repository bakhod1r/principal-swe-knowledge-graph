...
## 1. Definition
**Zero Downtime Migration Scalability Patterns** defines the horizontal sharding models, multi-tier caching architectures, and throughput scaling strategies for **Zero Downtime Migration**.
...
## 3. Usage
```yaml
# Kubernetes & Envoy Horizontal Autoscaling Topology for Zero Downtime Migration
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: zero-downtime-migration-autoscaler
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
    name: zero-downtime-migration-service
  minReplicas: 10
  maxReplicas: 200
  metrics:
  - type: Resource
