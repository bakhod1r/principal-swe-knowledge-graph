...
## 1. Definition
**Availability Monitoring Scalability Patterns** defines the horizontal sharding models, multi-tier caching architectures, and throughput scaling strategies for **Availability Monitoring**.
...
## 3. Usage
```yaml
# Kubernetes & Envoy Horizontal Autoscaling Topology for Availability Monitoring
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: availability-monitoring-autoscaler
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
    name: availability-monitoring-service
  minReplicas: 10
  maxReplicas: 200
  metrics:
  - type: Resource
