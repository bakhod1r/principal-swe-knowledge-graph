...
## 1. Definition
**Simpson Integration** represents a fundamental algorithmic paradigm and structural invariant within **19. Number Theory**.
...
## 2. Mental Model
```text
Invariant State Machine for Simpson Integration:
+--------------------+        State Transition        +--------------------+
...
## 3. Usage
```go
// Production pattern for Simpson Integration
type SimpsonIntegration struct {
    // Internal state descriptors and size invariants
    size int
}

func NewSimpsonIntegration() *SimpsonIntegration {
