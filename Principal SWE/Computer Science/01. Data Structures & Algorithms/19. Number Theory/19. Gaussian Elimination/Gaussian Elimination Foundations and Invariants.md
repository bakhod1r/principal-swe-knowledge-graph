...
## 1. Definition
**Gaussian Elimination** represents a fundamental algorithmic paradigm and structural invariant within **19. Number Theory**.
...
## 2. Mental Model
```text
Invariant State Machine for Gaussian Elimination:
+--------------------+        State Transition        +--------------------+
...
## 3. Usage
```go
// Production pattern for Gaussian Elimination
type GaussianElimination struct {
    // Internal state descriptors and size invariants
    size int
}

func NewGaussianElimination() *GaussianElimination {
