...
## 1. Definition
**Tonelli Shanks** represents a fundamental algorithmic paradigm and structural invariant within **19. Number Theory**.
...
## 2. Mental Model
```text
Invariant State Machine for Tonelli Shanks:
+--------------------+        State Transition        +--------------------+
...
## 3. Usage
```go
// Production pattern for Tonelli Shanks
type TonelliShanks struct {
    // Internal state descriptors and size invariants
    size int
}

func NewTonelliShanks() *TonelliShanks {
