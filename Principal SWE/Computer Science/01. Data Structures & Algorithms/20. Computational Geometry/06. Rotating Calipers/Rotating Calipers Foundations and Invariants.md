...
## 1. Definition
**Rotating Calipers** represents a fundamental algorithmic paradigm and structural invariant within **20. Computational Geometry**.
...
## 2. Mental Model
```text
Invariant State Machine for Rotating Calipers:
+--------------------+        State Transition        +--------------------+
...
## 3. Usage
```go
// Production pattern for Rotating Calipers
type RotatingCalipers struct {
    // Internal state descriptors and size invariants
    size int
}

func NewRotatingCalipers() *RotatingCalipers {
