...
## 1. Definition
**MST Kruskal Prim** represents a fundamental algorithmic paradigm and structural invariant within **11. Graphs**.
...
## 2. Mental Model
```text
Invariant State Machine for MST Kruskal Prim:
+--------------------+        State Transition        +--------------------+
...
## 3. Usage
```go
// Production pattern for MST Kruskal Prim
type MSTKruskalPrim struct {
    // Internal state descriptors and size invariants
    size int
}

func NewMSTKruskalPrim() *MSTKruskalPrim {
    return &MSTKruskalPrim{
        size: 0,
    }
}
```
