---
title: "Sprague Grundy Foundations and Invariants"
tags:
  - computer-science
  - algorithms
  - dsa
...
## 1. Definition
**Sprague Grundy** represents a fundamental algorithmic paradigm and structural invariant within **13. Dynamic Programming**.
...
## 2. Mental Model
```text
Invariant State Machine for Sprague Grundy:
+--------------------+        State Transition        +--------------------+
...
## 3. Usage
```go
// Production pattern for Sprague Grundy
type SpragueGrundy struct {
    // Internal state descriptors and size invariants
    size int
}

func NewSpragueGrundy() *SpragueGrundy {
    return &SpragueGrundy{
        size: 0,
    }
}
```
