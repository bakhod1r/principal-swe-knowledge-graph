...
## 1. Definition
**Offline LCA** represents a fundamental algorithmic paradigm and structural invariant within **12. Disjoint Set**.
...
## 2. Mental Model
```text
Invariant State Machine for Offline LCA:
+--------------------+        State Transition        +--------------------+
...
## 3. Usage
```go
// Production pattern for Offline LCA
type OfflineLCA struct {
    // Internal state descriptors and size invariants
    size int
}

func NewOfflineLCA() *OfflineLCA {
    return &OfflineLCA{
        size: 0,
    }
}
