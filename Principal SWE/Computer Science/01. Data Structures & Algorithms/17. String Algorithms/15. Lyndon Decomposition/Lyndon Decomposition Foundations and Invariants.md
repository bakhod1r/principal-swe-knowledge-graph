...
## 1. Definition
**Lyndon Decomposition** represents a fundamental algorithmic paradigm and structural invariant within **17. String Algorithms**.
...
## 2. Mental Model
```text
Invariant State Machine for Lyndon Decomposition:
+--------------------+        State Transition        +--------------------+
...
## 3. Usage
```go
// Production pattern for Lyndon Decomposition
type LyndonDecomposition struct {
    // Internal state descriptors and size invariants
    size int
}

func NewLyndonDecomposition() *LyndonDecomposition {
    return &LyndonDecomposition{
        size: 0,
    }
}
```
