...
## 1. Definition
**Burnside Polya** represents a fundamental algorithmic paradigm and structural invariant within **19. Number Theory**.
...
## 2. Mental Model
```text
Invariant State Machine for Burnside Polya:
+--------------------+        State Transition        +--------------------+
...
## 3. Usage
```go
// Production pattern for Burnside Polya
type BurnsidePolya struct {
    // Internal state descriptors and size invariants
    size int
}

func NewBurnsidePolya() *BurnsidePolya {
    return &BurnsidePolya{
        size: 0,
    }
}
```
