...
## 1. Definition
**Heavy Light Decomposition** represents a fundamental algorithmic paradigm and structural invariant within **11. Graphs**.
...
## 2. Mental Model
```text
Invariant State Machine for Heavy Light Decomposition:
+--------------------+        State Transition        +--------------------+
...
## 3. Usage
```go
// Production pattern for Heavy Light Decomposition
type HeavyLightDecomposition struct {
    // Internal state descriptors and size invariants
    size int
}

func NewHeavyLightDecomposition() *HeavyLightDecomposition {
