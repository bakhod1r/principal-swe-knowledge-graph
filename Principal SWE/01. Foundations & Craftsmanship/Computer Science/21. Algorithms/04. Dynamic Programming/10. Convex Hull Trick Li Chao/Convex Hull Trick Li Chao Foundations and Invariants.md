...
## 1. Definition
**Convex Hull Trick Li Chao** represents a fundamental algorithmic paradigm and structural invariant within **13. Dynamic Programming**.
...
## 2. Mental Model
```text
Invariant State Machine for Convex Hull Trick Li Chao:
+--------------------+        State Transition        +--------------------+
...
## 3. Usage
```go
// Production pattern for Convex Hull Trick Li Chao
type ConvexHullTrickLiChao struct {
    // Internal state descriptors and size invariants
    size int
}

func NewConvexHullTrickLiChao() *ConvexHullTrickLiChao {
    return &ConvexHullTrickLiChao{
        size: 0,
    }
}
```
