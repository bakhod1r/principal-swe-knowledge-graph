...
## 1. Definition
**LCS LIS** represents a fundamental algorithmic paradigm and structural invariant within **13. Dynamic Programming**.
...
## 2. Mental Model
```text
Invariant State Machine for LCS LIS:
+--------------------+        State Transition        +--------------------+
...
## 3. Usage
```go
// Production pattern for LCS LIS
type LCSLIS struct {
    // Internal state descriptors and size invariants
    size int
}

func NewLCSLIS() *LCSLIS {
    return &LCSLIS{
        size: 0,
    }
}
