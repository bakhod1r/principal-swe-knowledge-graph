...
Internal toolchain utilities: `compile -S`, `objdump`, `nm`, `cgo`, `pprof`, `trace`, `asm`, `link`, `buildid`, `covdata`, `dist`, and `addr2line`.
...
├── [[go tool covdata (Binary Code Coverage in Integration Tests)]]
├── [[go tool asm (Plan 9 Assembler)]]
├── [[go tool link (Linker)]]
├── [[go tool buildid (Build Identity)]]
├── [[go tool addr2line (Address Mapping)]]
├── [[go tool cgo (Bridge Stubs)]]
├── [[go tool dist (Bootstrap Toolchain & Architecture Verification)]]
└── [[go tool vet Analysis Passes & Custom Analyzers]]
```
...
- [[go tool covdata (Binary Code Coverage in Integration Tests)]] — Merging and analyzing code coverage data collected from running integration test binaries.
...
- [[go tool dist (Bootstrap Toolchain & Architecture Verification)]] — Building and bootstrapping the Go compiler toolchain.
- [[go tool vet Analysis Passes & Custom Analyzers]] — Driver architecture of static analysis passes in `go vet`.
