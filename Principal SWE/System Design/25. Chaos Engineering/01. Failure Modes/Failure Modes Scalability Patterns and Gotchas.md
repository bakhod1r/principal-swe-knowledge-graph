...
## 1. Definition
**Failure Modes Scalability Patterns** defines the horizontal sharding models, multi-tier caching architectures, and throughput scaling strategies for **Failure Modes**.
...
## 3. Usage
```yaml
# Kubernetes & Envoy Horizontal Autoscaling Topology for Failure Modes
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: failure-modes-autoscaler
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
    name: failure-modes-service
  minReplicas: 10
  maxReplicas: 200
  metrics:
  - type: Resource
