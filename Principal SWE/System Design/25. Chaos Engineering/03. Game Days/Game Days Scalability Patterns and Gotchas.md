...
## 1. Definition
**Game Days Scalability Patterns** defines the horizontal sharding models, multi-tier caching architectures, and throughput scaling strategies for **Game Days**.
...
## 3. Usage
```yaml
# Kubernetes & Envoy Horizontal Autoscaling Topology for Game Days
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: game-days-autoscaler
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
    name: game-days-service
  minReplicas: 10
  maxReplicas: 200
  metrics:
  - type: Resource
