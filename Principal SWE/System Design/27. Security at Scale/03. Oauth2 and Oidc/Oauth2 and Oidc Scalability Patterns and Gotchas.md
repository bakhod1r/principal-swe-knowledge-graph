...
## 1. Definition
**Oauth2 and Oidc Scalability Patterns** defines the horizontal sharding models, multi-tier caching architectures, and throughput scaling strategies for **Oauth2 and Oidc**.
...
## 3. Usage
```yaml
# Kubernetes & Envoy Horizontal Autoscaling Topology for Oauth2 and Oidc
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: oauth2-and-oidc-autoscaler
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
    name: oauth2-and-oidc-service
  minReplicas: 10
  maxReplicas: 200
  metrics:
  - type: Resource
