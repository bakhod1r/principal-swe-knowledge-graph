...
## 1. Definition
**Latency Budgets Scalability Patterns** defines the horizontal sharding models, multi-tier caching architectures, and throughput scaling strategies for **Latency Budgets**.
...
## 3. Usage
```yaml
# Kubernetes & Envoy Horizontal Autoscaling Topology for Latency Budgets
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: latency-budgets-autoscaler
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
    name: latency-budgets-service
  minReplicas: 10
  maxReplicas: 200
  metrics:
  - type: Resource
