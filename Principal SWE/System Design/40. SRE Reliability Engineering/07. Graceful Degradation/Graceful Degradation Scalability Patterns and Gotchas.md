...
## 1. Definition
**Graceful Degradation Scalability Patterns** defines the horizontal sharding models, multi-tier caching architectures, and throughput scaling strategies for **Graceful Degradation**.
...
## 3. Usage
```yaml
# Kubernetes & Envoy Horizontal Autoscaling Topology for Graceful Degradation
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: graceful-degradation-autoscaler
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
    name: graceful-degradation-service
  minReplicas: 10
  maxReplicas: 200
  metrics:
  - type: Resource
