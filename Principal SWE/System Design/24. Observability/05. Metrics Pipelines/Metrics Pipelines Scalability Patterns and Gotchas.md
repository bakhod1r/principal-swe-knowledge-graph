...
## 1. Definition
**Metrics Pipelines Scalability Patterns** defines the horizontal sharding models, multi-tier caching architectures, and throughput scaling strategies for **Metrics Pipelines**.
...
## 3. Usage
```yaml
# Kubernetes & Envoy Horizontal Autoscaling Topology for Metrics Pipelines
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: metrics-pipelines-autoscaler
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
    name: metrics-pipelines-service
  minReplicas: 10
  maxReplicas: 200
  metrics:
  - type: Resource
