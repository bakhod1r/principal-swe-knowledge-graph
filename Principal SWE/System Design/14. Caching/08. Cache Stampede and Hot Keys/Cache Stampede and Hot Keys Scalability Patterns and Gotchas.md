...
## 1. Definition
**Cache Stampede and Hot Keys Scalability Patterns** defines the horizontal sharding models, multi-tier caching architectures, and throughput scaling strategies for **Cache Stampede and Hot Keys**.
...
## 3. Usage
```yaml
# Kubernetes & Envoy Horizontal Autoscaling Topology for Cache Stampede and Hot Keys
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: cache-stampede-and-hot-keys-autoscaler
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
    name: cache-stampede-and-hot-keys-service
  minReplicas: 10
  maxReplicas: 200
  metrics:
  - type: Resource
