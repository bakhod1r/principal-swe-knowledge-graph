...
## 1. Definition
**SQL vs NoSQL Scalability Patterns** defines the horizontal sharding models, multi-tier caching architectures, and throughput scaling strategies for **SQL vs NoSQL**.
...
## 3. Usage
```yaml
# Kubernetes & Envoy Horizontal Autoscaling Topology for SQL vs NoSQL
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: sql-vs-nosql-autoscaler
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
    name: sql-vs-nosql-service
  minReplicas: 10
  maxReplicas: 200
  metrics:
  - type: Resource
