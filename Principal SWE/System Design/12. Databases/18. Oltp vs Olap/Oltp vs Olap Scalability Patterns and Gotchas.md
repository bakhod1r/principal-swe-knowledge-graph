...
## 1. Definition
**Oltp vs Olap Scalability Patterns** defines the horizontal sharding models, multi-tier caching architectures, and throughput scaling strategies for **Oltp vs Olap**.
...
## 3. Usage
```yaml
# Kubernetes & Envoy Horizontal Autoscaling Topology for Oltp vs Olap
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: oltp-vs-olap-autoscaler
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
    name: oltp-vs-olap-service
  minReplicas: 10
  maxReplicas: 200
  metrics:
  - type: Resource
