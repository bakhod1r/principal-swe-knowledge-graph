...
## 1. Definition
**Column Oriented Olap Scalability Patterns** defines the horizontal sharding models, multi-tier caching architectures, and throughput scaling strategies for **Column Oriented Olap**.
...
## 3. Usage
```yaml
# Kubernetes & Envoy Horizontal Autoscaling Topology for Column Oriented Olap
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: column-oriented-olap-autoscaler
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
    name: column-oriented-olap-service
  minReplicas: 10
  maxReplicas: 200
  metrics:
  - type: Resource
