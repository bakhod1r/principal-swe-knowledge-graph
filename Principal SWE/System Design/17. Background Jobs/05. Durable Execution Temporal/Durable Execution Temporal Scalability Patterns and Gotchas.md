...
## 1. Definition
**Durable Execution Temporal Scalability Patterns** defines the horizontal sharding models, multi-tier caching architectures, and throughput scaling strategies for **Durable Execution Temporal**.
...
## 3. Usage
```yaml
# Kubernetes & Envoy Horizontal Autoscaling Topology for Durable Execution Temporal
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: durable-execution-temporal-autoscaler
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
    name: durable-execution-temporal-service
  minReplicas: 10
  maxReplicas: 200
  metrics:
  - type: Resource
