...
## 1. Definition
**Cross Border and Fx Scalability Patterns** defines the horizontal sharding models, multi-tier caching architectures, and throughput scaling strategies for **Cross Border and Fx**.
...
## 3. Usage
```yaml
# Kubernetes & Envoy Horizontal Autoscaling Topology for Cross Border and Fx
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: cross-border-and-fx-autoscaler
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
    name: cross-border-and-fx-service
  minReplicas: 10
  maxReplicas: 200
  metrics:
  - type: Resource
