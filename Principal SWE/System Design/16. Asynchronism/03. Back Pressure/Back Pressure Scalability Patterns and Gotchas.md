...
## 1. Definition
**Back Pressure Scalability Patterns** defines the horizontal sharding models, multi-tier caching architectures, and throughput scaling strategies for **Back Pressure**.
...
## 3. Usage
```yaml
# Kubernetes & Envoy Horizontal Autoscaling Topology for Back Pressure
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: back-pressure-autoscaler
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
    name: back-pressure-service
  minReplicas: 10
  maxReplicas: 200
  metrics:
  - type: Resource
