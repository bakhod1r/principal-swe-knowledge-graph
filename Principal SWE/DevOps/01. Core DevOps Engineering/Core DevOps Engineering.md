...
End-to-end platform automation and SRE delivery: Operating systems, container runtimes, cloud networking, serverless compute, configuration management (Ansible), infrastructure provisioning, CI/CD pipelines, secret management (Vault), logging, metrics, GitOps (ArgoCD), service mesh (Istio), and cloud design patterns.
